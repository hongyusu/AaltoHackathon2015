...
Each track $T_i$ is then represented as a directed graph $G_i = (V_i,E_i)$ defined on a $2$D surface.
The vertex set $V_i=\{(x_{i,j},y_{i,j})\}_{j=1}^{m_i}$ includes all \gps\ locations of track $T_i$.
There exist an directed edge $e_{i,k}$ from vertex $v_{i,k}$ to $v_{i,k+1}$ $\forall k\le m_i-1$.
As a result, the edge set is defined as a collection of directed edges $E_i = \{e_{i,k}\}_{k=1}^{m_i-1}$.
