...
resultPath = '../result'
nodeInfoPath = path.join(resultPath, 'node_info.txt')#each line cotains:'id:lng lat alt'
mapInfoPath = path.join(resultPath, 'map_info.txt')#each line contains:'id1 id2:distance', the distance of unit is meter.
partialRoutesFile = path.join(toolPath, 'file_first_gps_london.txt')
